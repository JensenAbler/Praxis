...
try {
  ...
  const pc = await call('patronus_capabilities'); assert.equal(pc.name, 'Patronus');
  const pj = await call('patronus_jobs'); assert.ok(pj.jobs.length > 0);
  const completed = pj.jobs.find(j => j.state === 'succeeded' && j.artifacts.length);
  assert.ok(completed);
  const ps = await call('patronus_status', {jobId: completed.jobId}); assert.equal(ps.state, 'succeeded');
  const pa = await call('patronus_artifact', {jobId: completed.jobId, artifactId: completed.artifacts[0].artifactId, limit: 16});
  assert.equal(Buffer.from(pa.content, 'base64').length, 16);
  console.log(JSON.stringify({patronusMcpPassed:true, liveJobRead:completed.jobId, artifactBytesRead:16}));
