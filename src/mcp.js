...
export function createProbeServer({ jobs, audit, resourceUrl, bootId, release, authInfo, era }) {
  const owner = authInfo?.extra?.subject;
  if (owner !== 'jensen' || !authInfo?.scopes?.includes(SCOPE)) throw new Error('Authenticated owner required');
  const server = new McpServer({ name: 'Praxis Probe', version: VERSION }, {
  ...
  const register = (name, title, description, inputSchema, handler, readOnly = true, destructive = false) => {
    server.registerTool(name, {
      ...
      try {
        ...
        result = { ok: false, requestId, observedAt: new Date().toISOString(), error: {
          code, message: code === 'INTERNAL_ERROR' ? 'The fixture could not complete this operation.' : error.message,
