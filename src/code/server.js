    ...
    try {
      ...
      const data = req.body.action === 'capabilities' ? {
        ...
        usage: {
          ...
          jobRecovery: 'After job_start, call job_wait instead of polling job_status; use git_operation_wait and praxis_release_wait the same way for Git and release operations. Read job_status first for actual exit status, recent output, and retention facts. Use job_logs view=tail for recent output or query/stream for targeted retained records; complete log scans are optional.',
