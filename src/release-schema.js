...
export const releaseTools = {
  ...
  praxis_release_apply: { action: 'releaseApply', title: 'Activate a prepared Praxis release', write: true, destructive: true,
    description: 'Activate a ready, tested release plan through the independent updater. Requires its digest and expected current release. Active jobs defer activation. Failed health restores the previous application. This convenience activates the application slot; native root jobs can also maintain the gateway, helpers and configuration. If a saved operation reports restoration_failed, an explicit retry with identical inputs/key resumes that same restoration after observing current state.',
