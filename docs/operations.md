...
Runtime `coding-abc97268f6d9` (Praxis 0.4.0, source `abc97268f6d98bb80d72e388d3fb2a30b3d33205`) is active. [130 Linux tests](evidence/publication-v04-linux.json), actual local-Git authenticated integration, and [seven real-container MCP fixture groups](evidence/publication-v04-mcp.json) passed. The [deployment record](evidence/publication-v04-deployment.json) verifies all three HTTP services, the fixed deployment helper from the broker mount namespace, denied general sudo, and preservation of seven workspaces, 23 jobs, credentials, and existing journals. Final activation backup: `/root/praxis-probe-backups/coding-update-nkCsungy`.

The owner expanded authorization to podcast-discord publication and deployment. `praxis-git.service` runs as its own UID on loopback 8793, with protected configuration `/etc/praxis-git/config.json`, private state `/var/lib/praxis-git`, and a single-repository write deploy key. Shared source-only exchange directories live under `/srv/praxis-git-exchange`; command containers do not mount them. `/usr/local/libexec/praxis-deploy-discord` is root-owned and is the only no-argument sudo command permitted to the broker. It updates only `/opt/podcast-discord` and restarts only `podcast-discord.service`. The bot was adopted without source or configuration changes, then logged back into Discord. See [workflow and limits](publication-milestone.md).

The final operator updater SHA-256 is `5ff121e2a6f8a0f8f40309c67e9b46e861a0f011cc4e26797f3697878959f893`. It now includes the optional broker in idle checks, state preservation, release-only configuration updates, health checks, and rollback. Existing unfinished publication operations prevent release activation; databases are never restored over accepted work. The broker is enabled at boot. This is an owner-operated release updater, not an independent Praxis self-update tool.
