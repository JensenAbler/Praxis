...
The candidate passed [99 Linux tests](evidence/tool-quality-v03-linux.json) and [all seven authenticated real-container fixture groups](evidence/tool-quality-v03-mcp.json), including the noisy output case. The [deployment receipt](evidence/tool-quality-v03-deployment.json) records activation with the original stored data preserved. A subsequent native phone comparison is still needed to measure changes in call count, retries, response volume, and time. API fixture evidence is not a native-client performance result.
