...
Writes, exact-text patches, renames, and deletes require the current workspace revision and file hashes; a create uses `expectedSha256: null`. Multiple exact-text patches may target the same file, in order, each using the **original file hash**. Each `oldText` must match once in the text staged by earlier patches; the journal stores one final effect for that file. Mixed actions sharing a path and ancestor/descendant conflicts are rejected. All content and quota preconditions are checked before the journal is prepared. Small patches work on existing files up to the 2 MiB file limit; the 131,072-character input limit applies to each patch string and to full-file writes, not to a patched file's total characters. Commands and source operations use the same database lock; queued, starting, running, and canceling jobs exclude source reads, edits, diff review, and removal. `workspace_inspect` remains available during these jobs: it returns the last stored revision with `revisionVerified: false`, `dirty: null`, and active job IDs/statuses, without scanning files that the command may be changing. After the command ends, inspection verifies the current source revision again.
...
Diffs use jsdiff with three context lines, Git path quoting, executable modes, additions, deletions, and missing-newline markers. Binary and unsafe entries are identified without exposing their contents. A bounded comparison that cannot finish falls back to a complete file replacement; the response reports the fallback count. Per-file output is streamed into one private cache, reused only at the same workspace revision. Diff cursors count UTF-8 bytes; page boundaries preserve characters. Use `expectedRevision` on follow-up pages and copy returned cursors exactly. Omit the page budget for 16,384 bytes; `maxBytes` accepts 256–32,768 bytes. Legacy `limit` remains an alias with the same units; if both are supplied they must match. The first page includes up to 100 change summaries; the complete patch remains pageable.
