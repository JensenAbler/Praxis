...
# This updater changes no routes, services, project registrations, or runner image.
...
def state_snapshot(original=None):
    ...
    for name, (filename, owner, tables) in DATABASES.items():
        ...
        with contextlib.closing(sqlite3.connect(path.as_uri() + '?mode=ro', uri=True, timeout=5)) as db:
            ...
            db.execute('BEGIN')
            fingerprints = {table: table_fingerprint(db, table, original['databases'][name]['tables'][table]['columns'] if original else None) for table in tables}
...
def stop_idle_services(before_path):
    ...
    try:
        ...
        if jobs or operations or probe_jobs:
            raise RuntimeError(f'Update rejected: {jobs} active coding jobs, {operations} prepared operations, {probe_jobs} active probe jobs. Recover existing work first.')
        ...
        for unit in ('praxis-code.service', 'praxis-probe-worker.service'):
...
if __name__ == '__main__':
    ...
    if mode == 'stop-idle':
        ...
        print('Queues and mutation intents idle; coding and heartbeat worker stopped under admission reservation.')
    elif mode == 'verify':
...
finish() {
  ...
  if [[ $committed == 0 && $gateway_stopped == 1 ]]; then
    ...
    if [[ $candidate_started == 1 && $rollback_ok == 1 ]]; then
      ...
      python3 - "$backup" "$(basename -- "$previous")" <<'ROLLBACK_HEALTH' || rollback_ok=0
import json, pathlib, sys
base = pathlib.Path(sys.argv[1])
for name in ('rollback-gateway-health.json', 'rollback-coding-health.json'):
...
python3 - "$backup" "$release" "$(basename -- "$previous")" <<'RECEIPT'
