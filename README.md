...
**Praxis is connected in the owner's ChatGPT account at the canonical endpoint.** Its refreshed actions show 21 coding tools and nine diagnostic tools, and the explicit coding grant is confirmed through native consent and server records; see [connection evidence](docs/evidence/chatgpt-praxis-connection.json). The legacy connection was uninstalled. Actual native coding-tool execution remains pending. The first actual iPhone diagnostic run and fresh-conversation recovery passed; [the probe evidence](docs/evidence/iphone-probe-report.md) covers the nine diagnostic tools. No model API key is needed. The next step is [the coding phone test](docs/coding-phone-test.md); [repeatable diagnostic prompts](docs/phone-test.md) remain available for client/version checks.
